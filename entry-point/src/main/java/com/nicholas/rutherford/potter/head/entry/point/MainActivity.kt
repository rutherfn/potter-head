...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            mainContent(dependencies = dependencies, startupThemePreference = startupThemePreference)
        }
    }

    private fun mainContent(dependencies: ApplicationDependencies, startupThemePreference: Int) {
        setContent {
            val lifecycleOwner = LocalLifecycleOwner.current
            val themePreference by dependencies.dataStorePreferenceReader.readThemePreferenceValueFlow().collectAsState(initial = startupThemePreference)
            val systemInDarkTheme = isSystemInDarkTheme()
            val useDarkTheme = buildThemePreferenceFromValue(themePreference).isDarkTheme(systemInDarkTheme = systemInDarkTheme)

            LaunchedEffect(themePreference) { applyThemePreferenceNightMode(themePreferenceValue = themePreference) }
    ...
    private fun applyThemePreferenceNightMode(themePreferenceValue: Int) {
