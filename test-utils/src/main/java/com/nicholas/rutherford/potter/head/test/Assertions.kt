...
/**
 * Infix extension function to assert that a value is an instance of the specified type.
 * 
 * Usage:
 * ```
 * val exception = IllegalStateException("error")
 * exception shouldBeInstanceOf IllegalStateException::class
 * ```
 * 
 ...
 */
inline fun <reified T> Any?.shouldBeInstanceOf(clazz: Class<T>) {
    assertInstanceOf(clazz, this, "Expected value to be an instance of ${clazz.simpleName}")
}
